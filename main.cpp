...
void Stay(string, string);
void OneMore(string, string);
...
void BlackJackGame()
{
    ...
    FEHImage Warning;
    //if result = 0, tie, 1 means player won, -1 means player lost 
    int result = 2;
    int cord[2];
    int bank = 1000;
    int numGames = 0;
    ...
    if (PlayerTotal == 21){
        Sleep(3.0);
        LCD.WriteAt("Player 1 Wins",140,135); // Check if the values of the two cards are equal to 21 
        Sleep(5.0);
    }
    ...
    while(true){
        ...
        if(buttonPressed1.compare("Stay") == 0){
            ...
            if((DealerCard1Value+DealerCard2Value)<16){
                ...
                if((DealerTotal)>21){
                LCD.WriteAt("Player 1 Wins",140,135);
                Sleep(8.0);
                break;    
                }
                if (DealerTotal>PlayerTotal){
        ...
        while(buttonPressed1.compare("OneMore") == 0){
